...
```{r load decoupler, include = FALSE}
# load decoupleR
devtools::load_all()
# please specify the location of the inputs
input_location <- "./inst/benchdata/inputs/"
```
...
\
The prerequisites used in this tutorial can be found [here](https://drive.google.com/drive/folders/1p6_9XXuCvArCREakkecSMa5E72aH3wAD).
...
design_row <-
  tibble(
    ...
    # locations for the benchmark data, metadata, and set source
    bexpr_loc= file.path(input_location,"dorothea_bench_expr.rds"),
    bmeta_loc= file.path(input_location, "dorothea_bench_meta.rds"),
    source_loc= file.path(input_location, "networks/dorothea_filtered.rds"),
...
```{r bexpr_example}
readRDS(file.path(input_location, "dorothea_bench_expr.rds")) %>%
...
```{r bmeta_example}
readRDS(file.path(input_location,"dorothea_bench_meta.rds")) %>%
  slice_head(n=10) %>%
...
Please note that even though many resources would only contain `source` and `target` columns (e.g. GO term:Gene, or TF:target (in this example)), the decouple wrapper requires `mor`, `confidence`, and `likelihood` columns. As such, depending on the source set, arbitrary columns with these names should be provided. The `likelihood` column can be used to provide weights to each of the source:target interactions. Also, note that "tf", "target", and "confidence" can take any user-defined name (defined in the input tibble).
...
```{r source_example}
readRDS(file.path(input_location,"networks/dorothea_filtered.rds")) %>%
...
Then we make the assumption that 'True positives (TPs)' would get a 'score' from each statistical method that would rank them within the top `n` scores, where `n` = the number of experiments covered by the set resource. On the other hand, a 'True Negative (TN)' is defined as a TF that is not perturbed and is not ranked within the top `n` of TFs.
'Condition Negatives' are defined in two ways depending whether downsampling is performed. 
In the case of omitting downsampling (default when calculating [AUROC](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)), Condition Negatives are defined as the set of any TF not perturbed in the current experiment. As such, each of these non-perturbed sets is appended and considered as part of the condition negatives. Thus, this results in a very unequal proprotion of Condition positives (equal to the number of truly perturbed target in each experiment) and True Negatives (equal to the number of non-perturbed TFs for all of the experiments), thus vastly outnumbering Condition Positives.

When downsampling is performed, the number of Condition Negatives is set to be the same as the number of Condition Positives.
This is the default option when calculating the precision-recall curve, due to the nature of [precision](https://en.wikipedia.org/wiki/Precision_and_recall). Downsampling iterations are set to 1000 by default, but this value can be changed by the user.

In some cases, assuming that the perturbed target (in this case a TF) in a given experiment would be the most de-regulated one (i.e. the one with highest `score`) is a flawed assumption due to the nature of regulation in biology. For example, a TF correlated or downstream of the perturbed target might be more perturbed than the experiment target itself. However, at this stage, we believe that this is likely our best choice. Nonetheless, we are open to suggestions and implementations of alternative source set performance methods.


```{r bench_run, cache=TRUE, message=FALSE, warning=FALSE, echo=T, results='hide'}
dor_run <- run_benchmark(
  ...
  .downsample_pr = T, # downsample TNs for precision-recall curve
  .downsample_roc = F, # downsample TNs for ROC
  .downsample_times = 1000 # downsampling iterations
  )
```
