...
# Rename columns
k_des <- k_des %>%
  rename(source_col = gene_source,
         target_col = target,
         filter_crit = lvls,
         stats_list = statistics,
         opts_list = opts,
         bexpr_loc = bnch_expr,
         bmeta_loc = bench_meta,
         source_loc = net_loc) %>%
  mutate(filter_col = "Evidence")


# Rearrange columns
k_des <- k_des %>%
  select(set_name, bench_name,
         stats_list, opts_list,
         bexpr_loc, bmeta_loc, source_loc,
         source_col, target_col, filter_col, filter_crit)
k_des

# Try run
rm(kin_run)
kin_run <- run_benchmark(k_des, .minsize = 4, .silent = T, .downsample_pr = T,
                         .perform = F)
kin_run@summary$auroc_heat
kin_run@summary$summary_table
kin_run@bench_res
kin_run@design
...
design <- tibble(set_name="dorothea",
                 bench_name="dbd",
                 stats_list=list(c(
                   "mean",
                   "pscira",
                 ...
                 opts_list=list(list(scira = list(), pscira = list(), mean = list(),
                   viper = list(verbose = FALSE, minsize=0), gsva = list(verbose = FALSE))),
                 bexpr_loc="./inst/benchdata/inputs/dorothea_bench_expr.rds",
                 bmeta_loc="./inst/benchdata/inputs/dorothea_bench_meta.rds",
                 source_loc="./inst/benchdata/inputs/networks/dorothea_filtered.rds",
                 source_col="tf",
                 target_col="target",
...
design <- rbind(design,
                design %>%
                  mutate(source_loc = "./inst/benchdata/inputs/networks/dorothea_birewire.rds",
                         set_name = "birewire"))

design

dor_run <- run_benchmark(design, .downsample_times=1000)
saveRDS(dor_run, "./inst/benchdata/outputs/dor_dbd_upd.rds")
print(dor_run@summary$summary_table, n=50)
